...
class MacNoteAlphaShader extends FlxShader
{
	@:glFragmentSource('
#pragma header

uniform float downscroll;
...
void main()
{
	...
	vec4 base = custom_texture2D(bitmap, uv);

    base *= smoothstep(0.0, 0.2, downscroll == 1.0 ? uv.y : 1.0-uv.y);
